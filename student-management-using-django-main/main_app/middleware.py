...
class LoginCheckMiddleWare(MiddlewareMixin):
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if request.path.startswith('/admin/') or modulename.startswith('django.contrib.admin'):
            return None
        ...
        if user.is_authenticated:
            ...
            if (
                ...
                or request.path == reverse('start_registration')
                or request.path == reverse('complete_registration')
                or request.path == reverse('verify_totp')
                or request.path == '/verify_totp/'
